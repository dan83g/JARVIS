...
(function ($) {
  ...
  var ts = $.tablesorter,
    ...
    regexQuote: /([\n\t\x09\x0d\x0a]|<[^<]+>)/,
    // test if cell needs wrapping quotes
    regexBR: /(<br([\s\/])?>|\n)/g,
    // replace
    ...
    init: function (c) {
      c.$table.off(output.event).on(output.event, function (e) {
        e.stopPropagation(); // prevent multiple windows opening

        if (!output.busy && e.timeStamp - output.lastEvent > output.noDblClick) {
          output.lastEvent = e.timeStamp;
          output.busy = true; // explicitly use table.config.widgetOptions because we want
    ...
    processRow: function (c, $rows, isHeader, isJSON) {
      ...
      for (rowIndex = 0; rowIndex < rowsLength; rowIndex++) {
        ...
        for (indx = 0; indx < cellsLen; indx++) {
          $cell = $cells.eq(indx); // process rowspans
            ...
            for (col = 0; col < colspanLen; col++) {
              ...
              if ($cell.filter('[rowspan]').length) {
                ...
                for (row = 0; row < rowspanLen; row++) {
                  ...
                  tmpRow[rowIndex + row][cellIndex + col] = addSpanIndex ? wo.output_callbackJSON($cell, txt, cellIndex + col) || txt + '(' + (cellIndex + col) + ')' : isHeader ? txt : dupe ? txt : '';
                }
              } else {
                tmpRow[rowIndex][cellIndex + col] = addSpanIndex ? wo.output_callbackJSON($cell, txt, cellIndex + col) || txt + '(' + (cellIndex + col) + ')' : isHeader ? txt : dupe ? txt : '';
              }
            }
    ...
    process: function (c, wo, $rows, dump) {
      var mydata,
          ...
          $el = c.$table; // regex to look for the set separator or HTML

      wo.output_regex = new RegExp('(' + (/\\/.test(separator) ? '\\' : '') + separator + ')'); // make a list of hidden columns

      wo.output_hiddenColumnArray = [];

      for (indx = 0; indx < c.columns; indx++) {
        wo.output_hiddenColumnArray[indx] = $.inArray(indx, wo.output_ignoreColumns) > -1 || !wo.output_hiddenColumns && c.$headerIndexed[indx].css('display') === 'none' && !c.$headerIndexed[indx].hasClass('tablesorter-scroller-hidden-column');
      } // get header cells


      $this = $el.children('thead').children('tr').not('.' + (ts.css.filterRow || 'tablesorter-filter-row')).filter(function () {
        return wo.output_hiddenColumns || $(this).css('display') !== 'none';
      });
      headers = output.processRow(c, $this, true, outputJSON); // all tbody rows - do not include widget added rows (e.g. grouping widget headers)
      ...
      $rows = typeof saveRows === 'function' ? $rows.filter(saveRows) : // get (f)iltered, (v)isible, all rows (look for the first letter only), or jQuery filter selector
      /^f/.test(saveRows) ? $rows.not('.' + (wo.filter_filteredRow || 'filtered')) : /^v/.test(saveRows) ? $rows.filter(':visible') : // look for '.' (class selector), '#' (id selector),
      // ':' (basic filters, e.g. ':not()') or '[' (attribute selector start)
      /^[.#:\[]/.test(saveRows) ? $rows.filter(saveRows) : // default to all rows
      $rows; // process to array of arrays
      ...
      if (outputJSON) {
        ...
        if (wo.output_includeHeader) {
          tmp = [headers[len > 1 && wo.output_headerRows ? indx % len : len - 1]];
          tmpData = output.row2CSV(wo, wo.output_headerRows ? headers : tmp, outputArray).concat(output.row2CSV(wo, csvData, outputArray));
        } else {
    ...
    formatData: function (c, wo, $el, isHeader, colIndex) {
      ...
      result = wo.output_trimSpaces || isHeader ? $.trim(result) : result; // JSON & array outputs don't need quotes

      quotes = separator ? false : wo.output_wrapQuotes || wo.output_regex.test(result) || output.regexQuote.test(result);
      result = quotes ? '"' + result + '"' : result; // formatting callback - added v2.22.4
    ...
    popup: function (data, style, wrap) {
      ...
      try {
        generator.document.write('<html><head><title>' + output.popupTitle + '</title></head><body>' + '<textarea wrap="' + (wrap ? 'on' : 'off') + '" style="' + output.popupStyle + '">' + data + '\n</textarea>' + '</body></html>');
    ...
    download: function (c, wo, data) {
      ...
      var e,
          ...
          link = document.createElement('a'); // iOS devices do not support downloading. We have to inform user about
      // this limitation.
      ...
      if (gotBlob) {
        window.URL = window.URL || window.webkitURL; // prepend BOM for UTF-8 XML and text/* types (including HTML)
        // note: your browser will automatically convert UTF-16 U+FEFF to EF BB BF
        // see https://github.com/eligrey/FileSaver.js/blob/master/FileSaver.js#L68

        bom = /^\s*(?:text\/\S*|application\/xml|\S*\/\S*\+xml)\s*;.*charset\s*=\s*utf-8/i.test(wo.output_encoding) ? ['\ufeff', data] : [data];
        blob = new Blob(bom, {
          type: wo.output_encoding
        });
        ...
        if (nav.msSaveBlob) {
          ...
          link.download = wo.output_saveFileName; // Dispatching click event; using $(link).trigger() won't work

          if (document.createEvent) {
            e = document.createEvent('MouseEvents'); // event.initMouseEvent(type, canBubble, cancelable, view, detail, screenX, screenY, clientX, clientY,
  ...
  ts.addWidget({
    ...
    options: {
      ...
      // function(config, widgetOptions, data) { return data.content; }
...
// Module
___CSS_LOADER_EXPORT___.push([module.id, "/* .autowidth table {\n\twidth: auto;\t\n\ttable-layout: fixed;\n} */\n.autowidth .tablesorter td {\t\t\n\toverflow: hidden;\n\toverflow-x: auto;\n\t/* text-overflow: ellipsis;\t\t */\n\t/* white-space: nowrap; */\n\twhite-space: normal;\n\tmin-width: 64px;\n\tmax-width: 1200px;\n}\n.autowidth.htmltable {\n\twidth: auto!important;\n\toverflow: auto;\t\n}\n.autowidth .webix_template {\n\twidth: fit-content!important;\n}\n\n/*************\n  Blue Theme\n *************/\n/* overall */\n\n.tablesorter-blue {\n\t/* width: 100%; */\n\tbackground-color: #fff;\n\t/* margin: 10px 0 15px; */\n\ttext-align: left;\n\tborder-spacing: 0;\n\tborder: #cdcdcd 1px solid;\n\tborder-width: 1px 0 0 1px;\n}\n.tablesorter-blue th,\n.tablesorter-blue td {\n\tborder: #cdcdcd 1px solid;\n\tborder-width: 0 1px 1px 0;\n}\n\n/* header */\n.tablesorter-blue th,\n.tablesorter-blue thead td {\n\tfont: 12px/18px Arial, Sans-serif;\n\tfont-weight: bold;\n\tcolor: #000;\n\tbackground-color: #99bfe6;\n\tborder-collapse: collapse;\n\tpadding: 4px;\n\ttext-shadow: 0 1px 0 rgba(204, 204, 204, 0.7);\n}\n.tablesorter-blue tbody td,\n.tablesorter-blue tfoot th,\n.tablesorter-blue tfoot td {\n\tpadding: 4px;\n\tvertical-align: top;\n}\n.tablesorter-blue .header,\n.tablesorter-blue .tablesorter-header {\n\t/* black (unsorted) double arrow */\n\tbackground-image: url(data:image/gif;base64,R0lGODlhFQAJAIAAACMtMP///yH5BAEAAAEALAAAAAAVAAkAAAIXjI+AywnaYnhUMoqt3gZXPmVg94yJVQAAOw==);\n\t/* white (unsorted) double arrow */\n\t/* background-image: url(data:image/gif;base64,R0lGODlhFQAJAIAAAP///////yH5BAEAAAEALAAAAAAVAAkAAAIXjI+AywnaYnhUMoqt3gZXPmVg94yJVQAAOw==); */\n\t/* image */\n\t/* background-image: url(images/black-unsorted.gif); */\n\tbackground-repeat: no-repeat;\n\tbackground-position: center right;\n\tpadding: 4px 18px 4px 4px;\n\twhite-space: normal;\n\tcursor: pointer;\n}\n.tablesorter-blue .headerSortUp,\n.tablesorter-blue .tablesorter-headerSortUp,\n.tablesorter-blue .tablesorter-headerAsc {\n\tbackground-color: #9fbfdf;\n\t/* black asc arrow */\n\tbackground-image: url(data:image/gif;base64,R0lGODlhFQAEAIAAACMtMP///yH5BAEAAAEALAAAAAAVAAQAAAINjI8Bya2wnINUMopZAQA7);\n\t/* white asc arrow */\n\t/* background-image: url(data:image/gif;base64,R0lGODlhFQAEAIAAAP///////yH5BAEAAAEALAAAAAAVAAQAAAINjI8Bya2wnINUMopZAQA7); */\n\t/* image */\n\t/* background-image: url(images/black-asc.gif); */\n}\n.tablesorter-blue .headerSortDown,\n.tablesorter-blue .tablesorter-headerSortDown,\n.tablesorter-blue .tablesorter-headerDesc {\n\tbackground-color: #8cb3d9;\n\t/* black desc arrow */\n\tbackground-image: url(data:image/gif;base64,R0lGODlhFQAEAIAAACMtMP///yH5BAEAAAEALAAAAAAVAAQAAAINjB+gC+jP2ptn0WskLQA7);\n\t/* white desc arrow */\n\t/* background-image: url(data:image/gif;base64,R0lGODlhFQAEAIAAAP///////yH5BAEAAAEALAAAAAAVAAQAAAINjB+gC+jP2ptn0WskLQA7); */\n\t/* image */\n\t/* background-image: url(images/black-desc.gif); */\n}\n.tablesorter-blue thead .sorter-false {\n\tbackground-image: none;\n\tcursor: default;\n\tpadding: 4px;\n}\n\n/* tfoot */\n.tablesorter-blue tfoot .tablesorter-headerSortUp,\n.tablesorter-blue tfoot .tablesorter-headerSortDown,\n.tablesorter-blue tfoot .tablesorter-headerAsc,\n.tablesorter-blue tfoot .tablesorter-headerDesc {\n\t/* remove sort arrows from footer */\n\tbackground-image: none;\n}\n\n/* tbody */\n.tablesorter-blue td {\n\tcolor: #3d3d3d;\n\tbackground-color: #fff;\n\tpadding: 4px;\n\tvertical-align: top;\n}\n\n/* hovered row colors\n you'll need to add additional lines for\n rows with more than 2 child rows\n */\n.tablesorter-blue tbody > tr.hover > td,\n.tablesorter-blue tbody > tr:hover > td,\n.tablesorter-blue tbody > tr:hover + tr.tablesorter-childRow > td,\n.tablesorter-blue tbody > tr:hover + tr.tablesorter-childRow + tr.tablesorter-childRow > td,\n.tablesorter-blue tbody > tr.even.hover > td,\n.tablesorter-blue tbody > tr.even:hover > td,\n.tablesorter-blue tbody > tr.even:hover + tr.tablesorter-childRow > td,\n.tablesorter-blue tbody > tr.even:hover + tr.tablesorter-childRow + tr.tablesorter-childRow > td {\n\t/* background-color: #d9d9d9; */\n\tbackground-color: #faf8f8;\n}\n.tablesorter-blue tbody > tr.odd.hover > td,\n.tablesorter-blue tbody > tr.odd:hover > td,\n.tablesorter-blue tbody > tr.odd:hover + tr.tablesorter-childRow > td,\n.tablesorter-blue tbody > tr.odd:hover + tr.tablesorter-childRow + tr.tablesorter-childRow > td {\n\t/* background-color: #bfbfbf; */\n\tbackground-color: #e4edf8;\n}\n\n/* table processing indicator */\n.tablesorter-blue .tablesorter-processing {\n\tbackground-position: center center !important;\n\tbackground-repeat: no-repeat !important;\n\t/* background-image: url(images/loading.gif) !important; */\n\tbackground-image: url('data:image/gif;base64,') !important;\n}\n\n/* Zebra Widget - row alternating colors */\n.tablesorter-blue tbody tr.odd > td {\n\tbackground-color: #ebf2fa;\n}\n.tablesorter-blue tbody tr.even > td {\n\tbackground-color: #fff;\n}\n\n/* Column Widget - column sort colors */\n.tablesorter-blue td.primary,\n.tablesorter-blue tr.odd td.primary {\n\tbackground-color: #99b3e6;\n}\n.tablesorter-blue tr.even td.primary {\n\tbackground-color: #c2d1f0;\n}\n.tablesorter-blue td.secondary,\n.tablesorter-blue tr.odd td.secondary {\n\tbackground-color: #c2d1f0;\n}\n.tablesorter-blue tr.even td.secondary {\n\tbackground-color: #d6e0f5;\n}\n.tablesorter-blue td.tertiary,\n.tablesorter-blue tr.odd td.tertiary {\n\tbackground-color: #d6e0f5;\n}\n.tablesorter-blue tr.even td.tertiary {\n\tbackground-color: #ebf0fa;\n}\n\n/* caption */\n.tablesorter-blue > caption {\n\tbackground-color: #fff;\n}\n\n/* filter widget */\n.tablesorter-blue .tablesorter-filter-row {\n\tbackground-color: #eee;\n}\n.tablesorter-blue .tablesorter-filter-row td {\n\tbackground-color: #eee;\n\tline-height: normal;\n\ttext-align: center; /* center the input */\n\t-webkit-transition: line-height 0.1s ease;\n\t-moz-transition: line-height 0.1s ease;\n\t-o-transition: line-height 0.1s ease;\n\ttransition: line-height 0.1s ease;\n}\n/* optional disabled input styling */\n.tablesorter-blue .tablesorter-filter-row .disabled {\n\topacity: 0.5;\n\tfilter: alpha(opacity=50);\n\tcursor: not-allowed;\n}\n/* hidden filter row */\n.tablesorter-blue .tablesorter-filter-row.hideme td {\n\t/*** *********************************************** ***/\n\t/*** change this padding to modify the thickness     ***/\n\t/*** of the closed filter row (height = padding x 2) ***/\n\tpadding: 2px;\n\t/*** *********************************************** ***/\n\tmargin: 0;\n\tline-height: 0;\n\tcursor: pointer;\n}\n.tablesorter-blue .tablesorter-filter-row.hideme * {\n\theight: 1px;\n\tmin-height: 0;\n\tborder: 0;\n\tpadding: 0;\n\tmargin: 0;\n\t/* don't use visibility: hidden because it disables tabbing */\n\topacity: 0;\n\tfilter: alpha(opacity=0);\n}\n/* filters */\n.tablesorter-blue input.tablesorter-filter,\n.tablesorter-blue select.tablesorter-filter {\t\n\theight: auto;\n\tmargin: 0;\n\tpadding: 4px;\n\tbackground-color: #fff;\n\tborder: 1px solid #bbb;\n\tcolor: #333;\n\t-webkit-box-sizing: border-box;\n\t-moz-box-sizing: border-box;\n\tbox-sizing: border-box;\n\t-webkit-transition: height 0.1s ease;\n\t-moz-transition: height 0.1s ease;\n\t-o-transition: height 0.1s ease;\n\ttransition: height 0.1s ease;\n}\n/* rows hidden by filtering (needed for child rows) */\n.tablesorter .filtered {\n\tdisplay: none;\n}\n\n/* ajax error row */\n.tablesorter .tablesorter-errorRow td {\n\ttext-align: center;\n\tcursor: pointer;\n\tbackground-color: #e6bf99;\n}\n\n\n/* UPDATE THEME */\n.tablesorter-filter{\n\twidth: 100%;\n\tborder-radius: 6px; -webkit-border-radius: 6px;-moz-border-radius: 6px;\n}\n\n/* dark */\n.dark .tablesorter-header{\n\tbackground-color : #919394;\n\tcolor: #ffffff;\n\ttext-shadow:0 1px 25px whitesmoke;\t\n}\n\n.dark tbody tr.odd > td {\n\tbackground-color: #fcf9f9;\t\n}\n\n.dark tbody > tr.even:hover > td {\n\tbackground-color: #f5f5f5;\t\t\n}\n\n.dark tbody > tr.odd:hover > td {\n\tbackground-color: #f5f5f5;\t\t\n}\n\n/* compact */\n.compact .tablesorter-header{\n\tbackground: #3498db;\n\tcolor: #ffffff;\n\ttext-shadow:0 1px 25px whitesmoke;\n}\n\n/* flat */\n.flat .tablesorter-header{\n\tbackground: #3498db;\n\tcolor: #ffffff;\n\ttext-shadow:0 1px 25px whitesmoke;\n}\n\n/* contrast */\n.contrast .tablesorter-header{\n\tbackground: #191919;\n\tcolor: #ffffff;\n\ttext-shadow:0 1px 25px whitesmoke;\n}\n\n.contrast input.tablesorter-filter{\n\tbackground-color: #6e6c6c;\n\tcolor: #ffffff;\n}\n\n.contrast .tablesorter-filter-row td{\n\tbackground-color: #393939;\n}\n\n.contrast tbody tr.even > td {\n\tbackground-color: #6e6c6c;\n\tcolor: #ffffff;\t\n}\n\n.contrast tbody > tr.even:hover > td{\n\tbackground-color: #b3aeae;\n\tcolor: #ffffff;\t\n}\n\t\n.contrast tbody tr.odd > td {\n\tbackground-color: #393939;\n\tcolor: #ffffff;\t\t\n}\n\n.contrast tbody > tr.odd:hover > td {\n\tbackground-color: #b3aeae;\n\tcolor: #ffffff;\t\t\n}\n\n/* material */\n.material .tablesorter-header{\n\tbackground: #fdfcfc;\n\tcolor: #1ca1c1;\n\ttext-shadow:0 1px 25px whitesmoke;\n}\n\n.material input.tablesorter-filter{\n\tcolor: #1ca1c1;\n}\n\n.material .tablesorter-filter-row td{\n\tbackground-color: #fdfcfc;\n}\n\n.material tbody > tr > td {\t\n\tcolor: #1ca1c1;\t\t\n}\n\n/* mini */\n.mini .tablesorter-header{\n\tbackground: #fdfcfc;\n\tcolor: #1ca1c1;\n\ttext-shadow:0 1px 25px whitesmoke;\n}\n\n.mini input.tablesorter-filter{\n\tcolor: #1ca1c1;\n}\n\n.mini .tablesorter-filter-row td{\n\tbackground-color: #fdfcfc;\n}\n\n.mini tbody > tr > td {\t\n\tcolor: #1ca1c1;\t\t\n}", "",{"version":3,"sources":["webpack://./src/search/tablesorter/tablesorter.css"],"names":[],"mappings":"AAAA;;;GAGG;AACH;CACC,gBAAgB;CAChB,gBAAgB;CAChB,+BAA+B;CAC/B,yBAAyB;CACzB,mBAAmB;CACnB,eAAe;CACf,iBAAiB;AAClB;AACA;CACC,qBAAqB;CACrB,cAAc;AACf;AACA;CACC,4BAA4B;AAC7B;;AAEA;;cAEc;AACd,YAAY;;AAEZ;CACC,iBAAiB;CACjB,sBAAsB;CACtB,yBAAyB;CACzB,gBAAgB;CAChB,iBAAiB;CACjB,yBAAyB;CACzB,yBAAyB;AAC1B;AACA;;CAEC,yBAAyB;CACzB,yBAAyB;AAC1B;;AAEA,WAAW;AACX;;CAEC,iCAAiC;CACjC,iBAAiB;CACjB,WAAW;CACX,yBAAyB;CACzB,yBAAyB;CACzB,YAAY;CACZ,6CAA6C;AAC9C;AACA;;;CAGC,YAAY;CACZ,mBAAmB;AACpB;AACA;;CAEC,kCAAkC;CAClC,qIAAqI;CACrI,kCAAkC;CAClC,2IAA2I;CAC3I,UAAU;CACV,sDAAsD;CACtD,4BAA4B;CAC5B,iCAAiC;CACjC,yBAAyB;CACzB,mBAAmB;CACnB,eAAe;AAChB;AACA;;;CAGC,yBAAyB;CACzB,oBAAoB;CACpB,qHAAqH;CACrH,oBAAoB;CACpB,2HAA2H;CAC3H,UAAU;CACV,iDAAiD;AAClD;AACA;;;CAGC,yBAAyB;CACzB,qBAAqB;CACrB,qHAAqH;CACrH,qBAAqB;CACrB,2HAA2H;CAC3H,UAAU;CACV,kDAAkD;AACnD;AACA;CACC,sBAAsB;CACtB,eAAe;CACf,YAAY;AACb;;AAEA,UAAU;AACV;;;;CAIC,mCAAmC;CACnC,sBAAsB;AACvB;;AAEA,UAAU;AACV;CACC,cAAc;CACd,sBAAsB;CACtB,YAAY;CACZ,mBAAmB;AACpB;;AAEA;;;EAGE;AACF;;;;;;;;CAQC,+BAA+B;CAC/B,yBAAyB;AAC1B;AACA;;;;CAIC,+BAA+B;CAC/B,yBAAyB;AAC1B;;AAEA,+BAA+B;AAC/B;CACC,6CAA6C;CAC7C,uCAAuC;CACvC,0DAA0D;CAC1D,smBAAsmB;AACvmB;;AAEA,0CAA0C;AAC1C;CACC,yBAAyB;AAC1B;AACA;CACC,sBAAsB;AACvB;;AAEA,uCAAuC;AACvC;;CAEC,yBAAyB;AAC1B;AACA;CACC,yBAAyB;AAC1B;AACA;;CAEC,yBAAyB;AAC1B;AACA;CACC,yBAAyB;AAC1B;AACA;;CAEC,yBAAyB;AAC1B;AACA;CACC,yBAAyB;AAC1B;;AAEA,YAAY;AACZ;CACC,sBAAsB;AACvB;;AAEA,kBAAkB;AAClB;CACC,sBAAsB;AACvB;AACA;CACC,sBAAsB;CACtB,mBAAmB;CACnB,kBAAkB,EAAE,qBAAqB;CACzC,yCAAyC;CACzC,sCAAsC;CACtC,oCAAoC;CACpC,iCAAiC;AAClC;AACA,oCAAoC;AACpC;CACC,YAAY;CACZ,yBAAyB;CACzB,mBAAmB;AACpB;AACA,sBAAsB;AACtB;CACC,wDAAwD;CACxD,wDAAwD;CACxD,wDAAwD;CACxD,YAAY;CACZ,wDAAwD;CACxD,SAAS;CACT,cAAc;CACd,eAAe;AAChB;AACA;CACC,WAAW;CACX,aAAa;CACb,SAAS;CACT,UAAU;CACV,SAAS;CACT,6DAA6D;CAC7D,UAAU;CACV,wBAAwB;AACzB;AACA,YAAY;AACZ;;CAEC,YAAY;CACZ,SAAS;CACT,YAAY;CACZ,sBAAsB;CACtB,sBAAsB;CACtB,WAAW;CACX,8BAA8B;CAC9B,2BAA2B;CAC3B,sBAAsB;CACtB,oCAAoC;CACpC,iCAAiC;CACjC,+BAA+B;CAC/B,4BAA4B;AAC7B;AACA,qDAAqD;AACrD;CACC,aAAa;AACd;;AAEA,mBAAmB;AACnB;CACC,kBAAkB;CAClB,eAAe;CACf,yBAAyB;AAC1B;;;AAGA,iBAAiB;AACjB;CACC,WAAW;CACX,kBAAkB,EAAE,0BAA0B,CAAC,uBAAuB;AACvE;;AAEA,SAAS;AACT;CACC,0BAA0B;CAC1B,cAAc;CACd,iCAAiC;AAClC;;AAEA;CACC,yBAAyB;AAC1B;;AAEA;CACC,yBAAyB;AAC1B;;AAEA;CACC,yBAAyB;AAC1B;;AAEA,YAAY;AACZ;CACC,mBAAmB;CACnB,cAAc;CACd,iCAAiC;AAClC;;AAEA,SAAS;AACT;CACC,mBAAmB;CACnB,cAAc;CACd,iCAAiC;AAClC;;AAEA,aAAa;AACb;CACC,mBAAmB;CACnB,cAAc;CACd,iCAAiC;AAClC;;AAEA;CACC,yBAAyB;CACzB,cAAc;AACf;;AAEA;CACC,yBAAyB;AAC1B;;AAEA;CACC,yBAAyB;CACzB,cAAc;AACf;;AAEA;CACC,yBAAyB;CACzB,cAAc;AACf;;AAEA;CACC,yBAAyB;CACzB,cAAc;AACf;;AAEA;CACC,yBAAyB;CACzB,cAAc;AACf;;AAEA,aAAa;AACb;CACC,mBAAmB;CACnB,cAAc;CACd,iCAAiC;AAClC;;AAEA;CACC,cAAc;AACf;;AAEA;CACC,yBAAyB;AAC1B;;AAEA;CACC,cAAc;AACf;;AAEA,SAAS;AACT;CACC,mBAAmB;CACnB,cAAc;CACd,iCAAiC;AAClC;;AAEA;CACC,cAAc;AACf;;AAEA;CACC,yBAAyB;AAC1B;;AAEA;CACC,cAAc;AACf","sourcesContent":["/* .autowidth table {\n\twidth: auto;\t\n\ttable-layout: fixed;\n} */\n.autowidth .tablesorter td {\t\t\n\toverflow: hidden;\n\toverflow-x: auto;\n\t/* text-overflow: ellipsis;\t\t */\n\t/* white-space: nowrap; */\n\twhite-space: normal;\n\tmin-width: 64px;\n\tmax-width: 1200px;\n}\n.autowidth.htmltable {\n\twidth: auto!important;\n\toverflow: auto;\t\n}\n.autowidth .webix_template {\n\twidth: fit-content!important;\n}\n\n/*************\n  Blue Theme\n *************/\n/* overall */\n\n.tablesorter-blue {\n\t/* width: 100%; */\n\tbackground-color: #fff;\n\t/* margin: 10px 0 15px; */\n\ttext-align: left;\n\tborder-spacing: 0;\n\tborder: #cdcdcd 1px solid;\n\tborder-width: 1px 0 0 1px;\n}\n.tablesorter-blue th,\n.tablesorter-blue td {\n\tborder: #cdcdcd 1px solid;\n\tborder-width: 0 1px 1px 0;\n}\n\n/* header */\n.tablesorter-blue th,\n.tablesorter-blue thead td {\n\tfont: 12px/18px Arial, Sans-serif;\n\tfont-weight: bold;\n\tcolor: #000;\n\tbackground-color: #99bfe6;\n\tborder-collapse: collapse;\n\tpadding: 4px;\n\ttext-shadow: 0 1px 0 rgba(204, 204, 204, 0.7);\n}\n.tablesorter-blue tbody td,\n.tablesorter-blue tfoot th,\n.tablesorter-blue tfoot td {\n\tpadding: 4px;\n\tvertical-align: top;\n}\n.tablesorter-blue .header,\n.tablesorter-blue .tablesorter-header {\n\t/* black (unsorted) double arrow */\n\tbackground-image: url(data:image/gif;base64,R0lGODlhFQAJAIAAACMtMP///yH5BAEAAAEALAAAAAAVAAkAAAIXjI+AywnaYnhUMoqt3gZXPmVg94yJVQAAOw==);\n\t/* white (unsorted) double arrow */\n\t/* background-image: url(data:image/gif;base64,R0lGODlhFQAJAIAAAP///////yH5BAEAAAEALAAAAAAVAAkAAAIXjI+AywnaYnhUMoqt3gZXPmVg94yJVQAAOw==); */\n\t/* image */\n\t/* background-image: url(images/black-unsorted.gif); */\n\tbackground-repeat: no-repeat;\n\tbackground-position: center right;\n\tpadding: 4px 18px 4px 4px;\n\twhite-space: normal;\n\tcursor: pointer;\n}\n.tablesorter-blue .headerSortUp,\n.tablesorter-blue .tablesorter-headerSortUp,\n.tablesorter-blue .tablesorter-headerAsc {\n\tbackground-color: #9fbfdf;\n\t/* black asc arrow */\n\tbackground-image: url(data:image/gif;base64,R0lGODlhFQAEAIAAACMtMP///yH5BAEAAAEALAAAAAAVAAQAAAINjI8Bya2wnINUMopZAQA7);\n\t/* white asc arrow */\n\t/* background-image: url(data:image/gif;base64,R0lGODlhFQAEAIAAAP///////yH5BAEAAAEALAAAAAAVAAQAAAINjI8Bya2wnINUMopZAQA7); */\n\t/* image */\n\t/* background-image: url(images/black-asc.gif); */\n}\n.tablesorter-blue .headerSortDown,\n.tablesorter-blue .tablesorter-headerSortDown,\n.tablesorter-blue .tablesorter-headerDesc {\n\tbackground-color: #8cb3d9;\n\t/* black desc arrow */\n\tbackground-image: url(data:image/gif;base64,R0lGODlhFQAEAIAAACMtMP///yH5BAEAAAEALAAAAAAVAAQAAAINjB+gC+jP2ptn0WskLQA7);\n\t/* white desc arrow */\n\t/* background-image: url(data:image/gif;base64,R0lGODlhFQAEAIAAAP///////yH5BAEAAAEALAAAAAAVAAQAAAINjB+gC+jP2ptn0WskLQA7); */\n\t/* image */\n\t/* background-image: url(images/black-desc.gif); */\n}\n.tablesorter-blue thead .sorter-false {\n\tbackground-image: none;\n\tcursor: default;\n\tpadding: 4px;\n}\n\n/* tfoot */\n.tablesorter-blue tfoot .tablesorter-headerSortUp,\n.tablesorter-blue tfoot .tablesorter-headerSortDown,\n.tablesorter-blue tfoot .tablesorter-headerAsc,\n.tablesorter-blue tfoot .tablesorter-headerDesc {\n\t/* remove sort arrows from footer */\n\tbackground-image: none;\n}\n\n/* tbody */\n.tablesorter-blue td {\n\tcolor: #3d3d3d;\n\tbackground-color: #fff;\n\tpadding: 4px;\n\tvertical-align: top;\n}\n\n/* hovered row colors\n you'll need to add additional lines for\n rows with more than 2 child rows\n */\n.tablesorter-blue tbody > tr.hover > td,\n.tablesorter-blue tbody > tr:hover > td,\n.tablesorter-blue tbody > tr:hover + tr.tablesorter-childRow > td,\n.tablesorter-blue tbody > tr:hover + tr.tablesorter-childRow + tr.tablesorter-childRow > td,\n.tablesorter-blue tbody > tr.even.hover > td,\n.tablesorter-blue tbody > tr.even:hover > td,\n.tablesorter-blue tbody > tr.even:hover + tr.tablesorter-childRow > td,\n.tablesorter-blue tbody > tr.even:hover + tr.tablesorter-childRow + tr.tablesorter-childRow > td {\n\t/* background-color: #d9d9d9; */\n\tbackground-color: #faf8f8;\n}\n.tablesorter-blue tbody > tr.odd.hover > td,\n.tablesorter-blue tbody > tr.odd:hover > td,\n.tablesorter-blue tbody > tr.odd:hover + tr.tablesorter-childRow > td,\n.tablesorter-blue tbody > tr.odd:hover + tr.tablesorter-childRow + tr.tablesorter-childRow > td {\n\t/* background-color: #bfbfbf; */\n\tbackground-color: #e4edf8;\n}\n\n/* table processing indicator */\n.tablesorter-blue .tablesorter-processing {\n\tbackground-position: center center !important;\n\tbackground-repeat: no-repeat !important;\n\t/* background-image: url(images/loading.gif) !important; */\n\tbackground-image: url('data:image/gif;base64,') !important;\n}\n\n/* Zebra Widget - row alternating colors */\n.tablesorter-blue tbody tr.odd > td {\n\tbackground-color: #ebf2fa;\n}\n.tablesorter-blue tbody tr.even > td {\n\tbackground-color: #fff;\n}\n\n/* Column Widget - column sort colors */\n.tablesorter-blue td.primary,\n.tablesorter-blue tr.odd td.primary {\n\tbackground-color: #99b3e6;\n}\n.tablesorter-blue tr.even td.primary {\n\tbackground-color: #c2d1f0;\n}\n.tablesorter-blue td.secondary,\n.tablesorter-blue tr.odd td.secondary {\n\tbackground-color: #c2d1f0;\n}\n.tablesorter-blue tr.even td.secondary {\n\tbackground-color: #d6e0f5;\n}\n.tablesorter-blue td.tertiary,\n.tablesorter-blue tr.odd td.tertiary {\n\tbackground-color: #d6e0f5;\n}\n.tablesorter-blue tr.even td.tertiary {\n\tbackground-color: #ebf0fa;\n}\n\n/* caption */\n.tablesorter-blue > caption {\n\tbackground-color: #fff;\n}\n\n/* filter widget */\n.tablesorter-blue .tablesorter-filter-row {\n\tbackground-color: #eee;\n}\n.tablesorter-blue .tablesorter-filter-row td {\n\tbackground-color: #eee;\n\tline-height: normal;\n\ttext-align: center; /* center the input */\n\t-webkit-transition: line-height 0.1s ease;\n\t-moz-transition: line-height 0.1s ease;\n\t-o-transition: line-height 0.1s ease;\n\ttransition: line-height 0.1s ease;\n}\n/* optional disabled input styling */\n.tablesorter-blue .tablesorter-filter-row .disabled {\n\topacity: 0.5;\n\tfilter: alpha(opacity=50);\n\tcursor: not-allowed;\n}\n/* hidden filter row */\n.tablesorter-blue .tablesorter-filter-row.hideme td {\n\t/*** *********************************************** ***/\n\t/*** change this padding to modify the thickness     ***/\n\t/*** of the closed filter row (height = padding x 2) ***/\n\tpadding: 2px;\n\t/*** *********************************************** ***/\n\tmargin: 0;\n\tline-height: 0;\n\tcursor: pointer;\n}\n.tablesorter-blue .tablesorter-filter-row.hideme * {\n\theight: 1px;\n\tmin-height: 0;\n\tborder: 0;\n\tpadding: 0;\n\tmargin: 0;\n\t/* don't use visibility: hidden because it disables tabbing */\n\topacity: 0;\n\tfilter: alpha(opacity=0);\n}\n/* filters */\n.tablesorter-blue input.tablesorter-filter,\n.tablesorter-blue select.tablesorter-filter {\t\n\theight: auto;\n\tmargin: 0;\n\tpadding: 4px;\n\tbackground-color: #fff;\n\tborder: 1px solid #bbb;\n\tcolor: #333;\n\t-webkit-box-sizing: border-box;\n\t-moz-box-sizing: border-box;\n\tbox-sizing: border-box;\n\t-webkit-transition: height 0.1s ease;\n\t-moz-transition: height 0.1s ease;\n\t-o-transition: height 0.1s ease;\n\ttransition: height 0.1s ease;\n}\n/* rows hidden by filtering (needed for child rows) */\n.tablesorter .filtered {\n\tdisplay: none;\n}\n\n/* ajax error row */\n.tablesorter .tablesorter-errorRow td {\n\ttext-align: center;\n\tcursor: pointer;\n\tbackground-color: #e6bf99;\n}\n\n\n/* UPDATE THEME */\n.tablesorter-filter{\n\twidth: 100%;\n\tborder-radius: 6px; -webkit-border-radius: 6px;-moz-border-radius: 6px;\n}\n\n/* dark */\n.dark .tablesorter-header{\n\tbackground-color : #919394;\n\tcolor: #ffffff;\n\ttext-shadow:0 1px 25px whitesmoke;\t\n}\n\n.dark tbody tr.odd > td {\n\tbackground-color: #fcf9f9;\t\n}\n\n.dark tbody > tr.even:hover > td {\n\tbackground-color: #f5f5f5;\t\t\n}\n\n.dark tbody > tr.odd:hover > td {\n\tbackground-color: #f5f5f5;\t\t\n}\n\n/* compact */\n.compact .tablesorter-header{\n\tbackground: #3498db;\n\tcolor: #ffffff;\n\ttext-shadow:0 1px 25px whitesmoke;\n}\n\n/* flat */\n.flat .tablesorter-header{\n\tbackground: #3498db;\n\tcolor: #ffffff;\n\ttext-shadow:0 1px 25px whitesmoke;\n}\n\n/* contrast */\n.contrast .tablesorter-header{\n\tbackground: #191919;\n\tcolor: #ffffff;\n\ttext-shadow:0 1px 25px whitesmoke;\n}\n\n.contrast input.tablesorter-filter{\n\tbackground-color: #6e6c6c;\n\tcolor: #ffffff;\n}\n\n.contrast .tablesorter-filter-row td{\n\tbackground-color: #393939;\n}\n\n.contrast tbody tr.even > td {\n\tbackground-color: #6e6c6c;\n\tcolor: #ffffff;\t\n}\n\n.contrast tbody > tr.even:hover > td{\n\tbackground-color: #b3aeae;\n\tcolor: #ffffff;\t\n}\n\t\n.contrast tbody tr.odd > td {\n\tbackground-color: #393939;\n\tcolor: #ffffff;\t\t\n}\n\n.contrast tbody > tr.odd:hover > td {\n\tbackground-color: #b3aeae;\n\tcolor: #ffffff;\t\t\n}\n\n/* material */\n.material .tablesorter-header{\n\tbackground: #fdfcfc;\n\tcolor: #1ca1c1;\n\ttext-shadow:0 1px 25px whitesmoke;\n}\n\n.material input.tablesorter-filter{\n\tcolor: #1ca1c1;\n}\n\n.material .tablesorter-filter-row td{\n\tbackground-color: #fdfcfc;\n}\n\n.material tbody > tr > td {\t\n\tcolor: #1ca1c1;\t\t\n}\n\n/* mini */\n.mini .tablesorter-header{\n\tbackground: #fdfcfc;\n\tcolor: #1ca1c1;\n\ttext-shadow:0 1px 25px whitesmoke;\n}\n\n.mini input.tablesorter-filter{\n\tcolor: #1ca1c1;\n}\n\n.mini .tablesorter-filter-row td{\n\tbackground-color: #fdfcfc;\n}\n\n.mini tbody > tr > td {\t\n\tcolor: #1ca1c1;\t\t\n}"],"sourceRoot":""}]);
// Exports
...
/* harmony import */ var _node_modules_css_loader_dist_cjs_js_tablesorter_css__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(5716);
...
var update = _node_modules_style_loader_dist_runtime_injectStylesIntoStyleTag_js__WEBPACK_IMPORTED_MODULE_0___default()(_node_modules_css_loader_dist_cjs_js_tablesorter_css__WEBPACK_IMPORTED_MODULE_1__/* .default */ .Z, options);



/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (_node_modules_css_loader_dist_cjs_js_tablesorter_css__WEBPACK_IMPORTED_MODULE_1__/* .default.locals */ .Z.locals || {});
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.tab_view = exports.html_table_view = exports.iframe_view = exports.runAllQueries = exports.loadUrl = exports.loadHTMLData = exports.hide_tab_overlay = exports.clear_table = exports.try_del_tab = exports.del_tab = exports.logger = exports.ClearTimer = void 0;
const $ = __webpack_require__(9755);
const table_1 = __webpack_require__(5273);
const service_1 = __webpack_require__(628);
window.jQuery = jQuery;
// extend tabbar with 
webix.extend(webix.ui.tabbar.$protoWait[0], {
    defaults: {
        on: {
            onBeforeRender: function (data) {
                data.options.forEach(function (cell) {
...
//Ошибки в log
function logger(type, caption, message, log = 'log') {
    var _a, _b, _c;
    let log_config = {
    ...
    let log_text = `<font color=${(_b = (_a = log_config[type]) === null || _a === void 0 ? void 0 : _a.color) !== null && _b !== void 0 ? _b : 'gray'} class='fas ${(_c = log_config[type].icon_tag) !== null && _c !== void 0 ? _c : 'fa-question'}'>${caption}</font>: ${message}`;
    $$(log).add({ message: log_text });
}
exports.logger = logger;
function del_tab(tabbar_id, tabview_id, tab_id) {
    var _a, _b;
    // del tab
    (_a = $$(tabbar_id)) === null || _a === void 0 ? void 0 : _a.removeOption(tab_id);
    // delView
    (_b = $$(tabview_id)) === null || _b === void 0 ? void 0 : _b.removeView(tab_id);
}
exports.del_tab = del_tab;
// удаляем вкладку если это не вторичный поиск
function try_del_tab(tab_id, value) {
    if (value == undefined || value == '')
        del_tab("tabs", "tabview", tab_id);
}
exports.try_del_tab = try_del_tab;
function clear_table(table_id) {
    let table = $(`#tablesorter${table_id}`);
    if (table.length) {
...
const set_tabbar_focus = function () {
    let elements = document.getElementsByClassName('webix_item_tab');
    if ((elements === null || elements === void 0 ? void 0 : elements.length) > 0)
        elements[0].focus();
};
const get_tab_header = (icon_tag = 'database', caption) => `<span class='fas fa-${icon_tag}'> </span> ${caption}`;
function hide_tab_overlay(tabs, table) {
...
exports.hide_tab_overlay = hide_tab_overlay;
//HTMLData
function loadHTMLData(aTable, aValue = '', aNoCache = "false", tabs_id = 'tabs') {
    // set tabs
    var tabs = webix.$$(tabs_id);
    // clear table
    ...
    let promise = webix.ajax().headers({ "X-Requested-With": "XMLHttpRequest", "X-Cache-Bypass": aNoCache }).get(aTable.config.load_url, { "value": aValue }, function (text, xml, xhr) {
        ...
        try {
            ...
            if (aTable.config.count == 0) {
                ...
                // Заполняем данными                
                aTable.setHTML(table_1.json2table(oJson.data, 'tablesorter' + aTable.config.id, "tablesorter"));
    ...
    promise.catch(function (error) {
        ...
        if (aTable.config.isShedule && ((_a = aTable.config) === null || _a === void 0 ? void 0 : _a.isShedule) == true) {
            ClearTimer(aTable);
            aTable.config.timerId = setTimeout(loadHTMLData, aTable.config.timeOut, aTable, aValue, "true");
        }
    });
...
// URL
function loadUrl(aFrame, aValue = '') {
...
;
function iframe_view(query_id) {
    return {
        view: "iframe", id: `iframe${query_id}`, tab_id: query_id,
        load_url: service_1.Query.get_query_url(query_id),
        on: {
            onBeforeLoad: () => { $$(`iframe${query_id}`).showOverlay("<i class='fas fa-spinner fa-spin fa-blue fa-4x'></i><br><br><font size='4'><b>Загрузка страницы...</b></font>"); },
            onAfterLoad: () => {
                let iframe = $$(`iframe${query_id}`);
...
exports.iframe_view = iframe_view;
function html_table_view(caption, query_id, icon_tag) {
    return {
        view: "template",
        ...
        on: {
            ...
            onAfterRender: function () {
                $('#tablesorter' + this.config.id).tablesorter({
                    caption: caption,
                    theme: 'blue',
...
exports.html_table_view = html_table_view;
const get_view_id = (query_id, iframe) => (iframe == true ? `iframe${query_id}` : `datatable${query_id}`);
function tab_view(tab_caption, query_id, iframe, icon_tag) {
                    ...
                    cols: [
                        ...
                        {},
                        {
                            ...
                            click: function () {
                                ...
                                let active_tab_id = webix.$$('tabview').getTabbar().getValue();
                                let tables = $(`#tablesorterdatatable${active_tab_id}`);
                                let table = tables[0];
                                ...
                                onChange: function (newv, oldv) {
                                    if (this.config && this.config.view_id) {
                                        ...
                                        if (newv != -1) {
                                            ...
                                            var value = $$(this.config.textId).getValue();
                                            // HTMLTable
                                            if (entity.config.view == "template") {
                                                ClearTimer(entity);
...
const http_1 = __webpack_require__(488);
exports.Query = {
    get_query_url(query_id) {
        return http_1.full_url(`/search/query/${query_id}`);
    },
};
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.json2table = void 0;
function json2table(json, id, classes) {
...
const jQuery = __webpack_require__(9755);
window.jQuery = jQuery;
// tablesorter
...
const controls_1 = __webpack_require__(1115);
const func_1 = __webpack_require__(2195);
class SearchPage {
    ...
    init() {
        ...
        window.search = func_1.search;
        // custom scroll
        // webix.CustomScroll.init();
        // render main menu
        const main_menu = this.render_main_menu();
        // render controls 
        const main_view = this.render_main();
        // set user to label
        index_1.set_user_label($$('labelUser'), index_1.CONFIG);
        // run all queries
        controls_1.runAllQueries();
    }
    toolbar_view() {
        return {
            view: "toolbar",
    ...
    tabs_view() {
        ...
        this.server_data.queries.forEach(function (item) {
            tab_cells.push(controls_1.tab_view(item.name, item.id, item.iframe, item.icon_tag));
        });
    ...
    render_main() {
        return webix.ui({
            padding: 0,
            margin: 0,
            css: `${index_1.CONFIG.skin} webix_padding_0`,
            rows: [
                this.toolbar_view(),
                {
                    gravity: 3,
                ...
                },
                {
                    ...
                    headerHeight: 32,
                    collapsed: !index_1.CONFIG.errors_visible,
                    scroll: "y",
                    body: {
                        view: "list",
                        id: "log",
